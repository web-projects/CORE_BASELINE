...
bool TSYSCofInfo::isFirstCOFtrans() const
{
    return false;
}

char* TSYSCofInfo::getCMIndicator(char* buffer, unsigned int len) const
{
    ...
    if (isCIT) // Is a cardholder initiated transaction (CIT) ?
    {
        ...
        if (!done && cof_r && action_sale && store_y) // Added as case 1 of Story 42609
        {
    ...
    if (!done && (action_preauth || action_sale) && has_BillingId && !paymentNo)
    {
        if (cof_c)
        {
            tag = "M207"; // Cancellation == No show Charge ??
